...
use oauth2::{
    ...
    RevocationUrl, Scope, TokenUrl, url,
};
...
pub async fn get_oauth_url() -> Result<oauth2::url::Url, anyhow::Error> {
    ...
    let google_client_id = ClientId::new(
        ...
        config.google_calendar_credentials.client_id.to_string(),
    );
    let google_client_secret = ClientSecret::new(
        //env::var("GOOGLE_CLIENT_SECRET")
        //    .expect("Missing the GOOGLE_CLIENT_SECRET environment variable."),
        config.google_calendar_credentials.client_secret.to_string(),
    );
    ...
    for stream in listener.incoming() {
        if let Ok(mut stream) = stream {
            ...
            if state.secret() != csrf_state.secret() {
                return Err("Cross-site request forgery token mismatch".to_string());
            }
            ...
            //return Ok(vec![code_secret, state.secret().to_string()]);
            return Ok(Token {
                code: code_secret,
